...
if(couponCode==="PRIME200" && getTotalCost()>500){
  ...
  useEffect(() => {
    const fetchFoodItemDetails = () => {
      console.log(typeof foodItemDetails);
      const details ={};
      for (const itemId in cartItems) {
        const { name, price, picture, description, _id } = cartItems[itemId];
        if (name && _id) {
          details[itemId] = { _id, name, picture, price, description, quantity: cartItems[itemId].quantity };
        }
      }
    ...
    <div className="">
      ...
      {Object.entries(foodItemDetails).map(([itemId, item]) => (
        <div key={itemId}>
         ...
         <p>Rs:{item.price}</p>
          <p><FontAwesomeIcon icon={faMinus} className="cursor-pointer text-white bg-red-500 p-1 rounded-3xl" onClick={()=>handleRemoveQuantity(itemId)}/> <span className="border-2 p-1">{item.quantity}</span><FontAwesomeIcon icon={faPlus} onClick={()=>handleAddToCart(itemId)} className="bg-red-500 text-white p-1 rounded-3xl cursor-pointer"/></p>
...
<button onClick={handleCheckout} className="bg-red-500 text-white rounded-xl p-2">Procced to CheckOut</button>
...
<h2>Use Code <span className="font-bold text-2xl text-red-500" id="coupon" value={couponCode}>WELCOME100 </span><FontAwesomeIcon icon={faCopy} onClick={()=>{setCouponCode("WELCOME100");alert("coupon code copied sucessfully")}}  className=" cursor-pointer text-black text-lg ml-3"/></h2>
